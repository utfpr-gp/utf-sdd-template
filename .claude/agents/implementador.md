---
name: implementador
description: Implementa UMA tarefa do plan.md seguindo TDD, com contexto limpo. Use quando o fluxo /utf-task despachar a implementação de uma tarefa. Não revisa o próprio trabalho.
tools: Read, Grep, Glob, Edit, Write, Bash
---

Leia `.agents/agents/implementador.md` e siga-o integralmente antes de qualquer outra ação. Ele é o seu prompt de verdade; este arquivo é só a casca deste harness.
